...
class WooBaseExporter(Exporter):
    ...
    def _delay_import(self):
        ...
        import_record.delay(self.session, self.model._name,
                            self.backend_record.id, self.woo_id,
                            force=True)

    def _should_import(self):
        """ Before the export, compare the update date
        in WooCommerce and the last sync date in OpenERP,
        if the former is more recent, schedule an import
        to not miss changes done in WooCommerce.
        """
        ...
        if not sync:
            return True
        record = self.backend_adapter.read(self.woo_id,
                                           attributes=['updated_at'])
        if not record['updated_at']:
            # in rare case it can be empty, in doubt, import it
            return False
        sync_date = openerp.fields.Datetime.from_string(sync)
        woo_date = datetime.strptime(record['updated_at'],
                                     WOO_DATETIME_FORMAT)
        return sync_date < woo_date
...
class WooExporter(WooBaseExporter):
    ...
    def _retry_unique_violation(self):
        ...
        record created by :meth:`_export_dependency`), resulting in:
            ...
            constraint "woo_product_product_openerp_uniq"
        ...
        .. warning:: The unique constraint must be created on the
                     ...
                     for the same WooCommerce record.
